...
### 5. Unknown player fallback (updated 22 May 2026)
Players absent from rolling stats (qualifiers, wildcards, debutants) previously received a flat `p = 0.45` against any opponent — nearly a coin flip. This has been replaced with an Elo-based fallback:

- If the player has an Elo rating: use it (global + surface blend)
- If the player has no Elo rating (true newcomer): assign the database median (≈ 1500)

Effect: a wildcard with no data now has ~5% vs a top-10 player instead of 45%, eliminating unrealistic simulation outcomes where low-ranked players win Grand Slams.
...
## FO26 predictions (22 May 2026) — real draw

*Updated with the official Roland-Garros 2026 draw released 22 May 2026. Tournament begins 24 May.*
...
| Paul T. | 2.4% |
| Fritz T. | 2.1% |
| Auger-Aliassime F. | 2.0% |

*Note: Alcaraz absent from the draw. Full standings: `results/fo26/atp_probs.csv`*
...
## Drop Shot — interactive web app

The predictions from this pipeline power **[Drop Shot](https://github.com/maanitmehta/drop-shot)**, a public-facing web app built on top of this repo.

Features:
- **Bracket simulator** — generates a unique R32-onwards bracket per run (R128/R64 simulated silently)
- **Monte Carlo odds** — 3,000-run title probability leaderboard
- **Head-to-head predictor** — pick any two players, get a win probability + clay Elo insight
- **Compare** — run two independent simulations side by side to visualise variance
- **Player card** — click any player in the bracket for their Elo rating and recent win rate

Built with Flask + vanilla JS, deployed on Railway. Source: [github.com/maanitmehta/drop-shot](https://github.com/maanitmehta/drop-shot)
