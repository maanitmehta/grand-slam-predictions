...
# Elo rating assigned to players with no history in the database.
# ~1500 = median tour player; gives them ~4% vs Sinner/Swiatek, ~24% vs a fringe tour player.
...
def predict_match(A, B, tour="atp", tournament="ao26"):
    ...
    if A not in stats.index or B not in stats.index:
        # Fall back to Elo-only prediction.
        # Players absent from Elo get _FALLBACK_ELO (~median tour rating),
        # which gives wildcards/qualifiers a realistic ~4% vs a top-10 player.
        a_global  = _get_elo(A, elo, "elo_global")
        b_global  = _get_elo(B, elo, "elo_global")
        a_surface = _get_elo(A, elo, f"elo_{surface}")
        b_surface = _get_elo(B, elo, f"elo_{surface}")
        p = 0.5 * _elo_prob(a_global, b_global) + 0.5 * _elo_prob(a_surface, b_surface)
